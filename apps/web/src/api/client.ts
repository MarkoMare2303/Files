 ...
 * mit `fetch`.
 *
 * Gegenüber der nativen Fassung kommt eine Besonderheit des Browsers hinzu:
 * `navigator.onLine` ist zwar unzuverlässig positiv (WLAN ohne Internet meldet
 * „online"), aber zuverlässig negativ. Ein `false` wird deshalb als sicheres
 * Offline-Signal genutzt, ein `true` als nichtssagend behandelt.
 */
...
/** Sicheres Offline-Signal: nur ein explizites `false` zählt. */
...
export async function apiRequest<T>(path: string, options: RequestOptions = {}): Promise<T> {
  ...
  try {
    const response = await fetch(buildUrl(path, options.query), {
      ...
      // Antworten auf angemeldete Anfragen dürfen nie in einem geteilten
      // Browser-Cache landen (§42).
      cache: 'no-store',
      credentials: 'omit',
    });
    ...
    if (!response.ok) {
      const error = (
        payload as { error?: { code?: string; userMessage?: LocalizedText; details?: unknown } }
      )?.error;
      throw new ApiError(
        error?.code ?? 'UNKNOWN',
        response.status,
